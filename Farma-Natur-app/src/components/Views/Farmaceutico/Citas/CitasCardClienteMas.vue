<template>
  <div class="card-cita">
    ...
    <div class="card-data">
      <span v-if="cliente">{{ cliente.nombre }}</span>
      <span v-else>Cargando...</span>
    </div>
    <div class="card-subdata" v-if="cliente">
      Total citas: <b>{{ cliente.totalCitas }}</b>
    </div>
  </div>
</template>
...
import { getClienteConMasCitas } from '../services/citasServices.js';
...
onMounted(async () => {
  try {
    ...
    cliente.value = { nombre: 'No disponible', totalCitas: '-' };
  }
});
</script>
